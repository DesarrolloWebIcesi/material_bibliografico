...
<div id="form_solic_audiovisual">
	...
	<!-- Se usa la función javascript validarFormularioAudiovisual para no tener que poner una comparación de cada campo en el onSubmit -->
	<form action="<?php echo getenv('REQUEST_URI'); ?>" name="formulario" method="post" onsubmit="return validarFormularioAudiovisual();">
	  <table border="1" align="center" cellpadding="4" cellspacing="0">
        <tr bgcolor="#CCCCCC">
          <td colspan="2"><b>Formulario de solicitud de material audiovisual</b></td>
        </tr>
		
		<tr>
		  <td class="obligatorio">Profesor que solicita el material*</td>
          <td>
		...
		<tr>
          <td class="obligatorio">Tipo de material*</td>
          <td>
		    <!--<input name="tipoMaterial" id="tipoMaterial" type="text" size="50" maxlength="700" value="<?php echo $tipoMaterial?>"/>-->
			<select name="tipoMaterial" id="tipoMaterial">
              <option value="Casete" <?php echo ($tipoMaterial == "Casete" ? "selected" : ""); ?>>Casete</option>
              <option value="CD" <?php echo ($tipoMaterial == "CD" ? "selected" : ""); ?>>CD</option>
              <option value="Diapositiva" <?php echo ($tipoMaterial == "Diapositiva" ? "selected" : ""); ?>>Diapositiva</option>
              <option value="DVD" <?php echo ($tipoMaterial == "DVD" ? "selected" : ""); ?>>DVD</option>
              <option value="Videograbación (VHS y BETA)" <?php echo ($tipoMaterial == "Videograbación (VHS y BETA)" ? "selected" : ""); ?>>Videograbaci&oacute;n (VHS y BETA)</option>
			<!-- Si existe un arreglo $camposError y tiene un valor asignado con el nombre del campo es porque el formulario fue validado y se encontró un error en el valor del campo -->
			<?php
			  if (isset($camposError['tipoMaterial']))
			    echo "<br/><div class='error'>".$camposError['tipoMaterial']."</div>";
			?>
		  </td>
        </tr>
		<tr>
	  ...
	  <p align="center">
		...
		<input type="hidden" name="tipo" id="tipo" value="audiovisual" />
		<input type="hidden" name="tipoSolicitud" id="tipoSolicitud" value="Material audiovisual" />
		&nbsp;
