...
<div id="form_solic_suscripcion">
	...
	<!-- Se usa la función javascript validarFormularioSuscripcion para no tener que poner una comparación de cada campo en el onSubmit -->
	<form action="<?php echo getenv('REQUEST_URI'); ?>" name="formulario" method="post" onsubmit="return validarFormularioSuscripcion();">
	  <table border="1" align="center" cellpadding="4" cellspacing="0">
        <tr bgcolor="#CCCCCC">
          <td colspan="2"><b>Formulario de solicitud de suscripciones</b></td>
        </tr>
		<tr>
		  <td class="obligatorio">Profesor que solicita la subscripci&oacute;n*</td>
          <td>
		...
		<tr>
          <td class="opcional">ISSN<br/><div class="isxn">Sin guiones ni espacios</div></td>
          <td>
		    <input name="issn" id="issn" type="text" size="8" maxlength="8" value="<?php echo $issn?>"/>
			<!-- Si existe un arreglo $camposError y tiene un valor asignado con el nombre del campo es porque el formulario fue validado y se encontró un error en el valor del campo -->
			<?php
			  if (isset($camposError['issn']))
			    echo "<br/><div class='error'>".$camposError['issn']."</div>";
			?>
		  </td>
        </tr>
		<tr>
	  ...
	  <p align="center">
		...
		<input type="hidden" name="tipo" id="tipo" value="suscripcion" />
		<input type="hidden" name="tipoSolicitud" id="tipoSolicitud" value="Suscripciones" />
		&nbsp;
