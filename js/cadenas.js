//Estas funciones las dio Web, por lo que no se modificaron
...
function isInteger(theElement, theElementName)
{
  ...
  if (filter.test(s))  
       return true;
         ...
         alert("Por favor inserte un número entero válido en el campo " +  theElementName);
    theElement.focus(); 
    theElement.select(); 
    return false;
}
      ...
      { 
         for (var i=0; (i <= s.length && s.charAt(i) != '.'); )
         {
          if (((s.charAt(i) >= 0) &&(s.charAt(i) <= 9)) || (s.charAt(i) == ',' && i != 0 && i != s.length-1) || (s.charAt(i) == '.') )
           ...
           { 
             alert( "El campo " +theElementname +  " debe contener sólo números" ); 
             theElement.focus(); 
             theElement.select(); 
             return false; 
           } 
...
function validarCadena (campo,name,longmin,nulo,tipodato)
{
 if (name.length==0)
  ...
  {
    window.alert("La longitud del campo "+vmostrar+" no es válida");
    campo.focus(); 
    campo.select(); 
    return false;
  }
...
/*function mail(texto){
	...
	alert("ingrese un correo valido "+mailres);
    return mailres;
} */